package com.nh.dataspider.redis.util;
...
//        allKey.addAll(Objects.requireNonNull(redisTemplate.keys("*" + cacheKeyUtil.getVisiualData() + "*")));
    ...
    /**
     * 删除包含特定字符串的key(不确定的key)
     * @param uncertainKey	不确定的key(key的部分内容)
     */
    public void removeUncertainKey(String uncertainKey) {
    	Set<String> allKeys = getAllKeys();
    	for (String key : allKeys) {
    		if(key.contains(uncertainKey)) {
    			remove(key);
    		}
		}
    }
