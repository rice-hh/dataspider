...
public class LunarToSolar {
     ...
     */
    public static String lunarToSolar(String dateStr) {
    	
    	Date date = DateUtil.stringToDates(dateStr);
    	//农历年份
    	int year = toCalendar(date).get(Calendar.YEAR);
    	//农历月
    	int month = toCalendar(date).get(Calendar.MONTH)+ 1;
    	//农历日
    	int monthDay = toCalendar(date).get(Calendar.DAY_OF_MONTH);
    	//该月是否是闰月
    	boolean isLeapMonth = getLunarLeapMonth(year)==month;
        ...
        solarInfo[0] = year;
        return solarInfo[0]+"-"+(solarInfo[1]<10?("0"+solarInfo[1]):solarInfo[1])+"-"+(solarInfo[2]<10?("0"+solarInfo[2]):solarInfo[2]);
    }
     ...
     * @return 闰月
     */
    private static int getLunarLeapMonth(int iYear) { 
        char iMonth = iLunarLeapMonthTable[(iYear - MIN_YEAR) / 2]; 
    ...
    private static void main(String[] args) {
        System.out.println(lunarToSolar("2023-08-19"));
    }
}
